/**
 * servicioGeneracionPdfLegacy.ts
 * 
 * Implementacion original del motor de generacion de PDFs (preservada durante Ola 2B).
 * Este archivo NO debe modificarse. Sirve como fallback seguro durante la refactorizacion.
 * 
 * TODO Ola 2B: Una vez completado el motor modular v2 con feature flag canary,
 * este legado puede archivarse o eliminarse tras validacion de paridad completa.
 */
...
export async function generarPdfExamen({
  ...
  while (numeroPagina <= paginasObjetivo && (numeroPagina === 1 || indicePregunta < totalPreguntas)) {
    ...
    if (!folioEnEncabezado) {
      page.drawText(qrTexto, { x: margen, y: margen - 16, size: 8.5, font: fuenteBold, color: colorPrimario });
